...
/**
 * Fetches and displays the TV show cast, making each member clickable.
 * @param {string} tvId - The ID of the TV show.
 */
async function displayTvCast(tvId) {
    const castContainer = document.getElementById('cast-container');
    if (!castContainer) return;

    try {
        const response = await fetch(`/api/tv/${tvId}/cast`);
...
/**
 * Fetches and displays watch provider information for the TV show.
 * @param {string} tvId - The ID of the TV show.
 */
async function displayWatchProviders(tvId) {
    const container = document.getElementById('platform-container');
    if (!container) return;

    try {
        const response = await fetch(`/api/tv/${tvId}/platforms`);
        const providersIndia = await response.json();
        const uniqueProviders = new Map();

        // Helper function to process providers and avoid duplicates
        ...
        if (uniqueProviders.size === 0) {
             container.innerHTML = '<p class="text-gray-400">Not available on major streaming platforms in this region.</p>';
             return;
        }
...
/**
 * Handles the click event of the "Submit Review" button for TV shows.
 */
async function submitTvReview() {
    ...
    const tvId = window.location.pathname.split('/').pop();

    try {
        const response = await fetch(`/tv/${tvId}/review`, {
            method: 'POST',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tvId && !isNaN(tvId)) {
        ...
        if (submitBtn) {
            submitBtn.addEventListener('click', submitTvReview);
        }
    } else {
        console.error("Could not find a valid TV Show ID in the URL.");
    }
});
